    ...
    def cast_ray(self, orig, dir, sceneObj = None, recursion = 0):
        ...
        if material.matType == OPAQUE:
            for light in self.lights:
                ...
                finalColor = lb.add(finalColor, lightColor)
            ...
            for light in self.lights:
                specColor = lb.add(specColor, light.getSpecColor(intersect, self))

            finalColor = reflectColor + specColor

        elif material.matType == TRANSPARENT:
            outside = lb.dot(dir, intersect.normal) < 0
            ...
            reflect = reflectVector(intersect.normal, np.array(dir) * -1)
            reflectOrig = lb.add(intersect.point, bias) if outside else lb.suv1stract(intersect.point, bias)
            ...
            if kr < 1:
                ...
                refractOrig = lb.suv1stract(intersect.point, bias) if outside else lb.add(intersect.point, bias)
